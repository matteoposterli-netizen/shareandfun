-- Fix cancel_booking: la migrazione 20260524000000_ombrellone_codice_mappa.sql ha
-- rimosso le colonne fila e numero da ombrelloni (ora c'è solo codice).
-- La funzione cancel_booking usava ancora SELECT fila, numero causando
-- "column fila does not exist".
...
BEGIN
  ...
  FOR v_item IN
    ...
    WHERE d.id = ANY(p_disp_ids)
  LOOP
    SELECT codice, credito_giornaliero INTO v_omb
    ...
    IF v_omb.codice IS NOT NULL THEN
      v_omb_short := v_omb.codice;
