...
> Tutte le migrazioni in `supabase/migrations/` sono state applicate sul DB di produzione. **Ultima migration applicata**: `20260608140000_aggiorna_mappa_ombrelloni.sql` (drop tabella `whatsapp_config`, resa obsoleta dalla gestione centralizzata via env var). Inclusa la `20260524000000_ombrellone_codice_mappa.sql` (aggiunta `codice`/`pos_x`/`pos_y` a `ombrelloni`, `mappa_passerelle` a `stabilimenti`, drop `fila`/`numero`) — applicata in produzione. Cumulative su `reset_stagione`: la 500000 rimpiazza la 300000 (reset dello stato di registrazione + cleanup auth.users per user_id), la 20260426000000 rimpiazza la 500000 (estende la cleanup auth.users anche al match per email + introduce la RPC `unblock_invito_email`).
>
...
- `aggiorna_mappa_ombrelloni(p_stabilimento_id, p_deletes uuid[], p_updates jsonb, p_inserts jsonb, p_passerelle jsonb)` — salva atomicamente la mappa di uno stabilimento dall'editor `mappa-builder`. Two-phase per evitare conflitti del vincolo unique `(stabilimento_id, pos_x, pos_y)` e `(stabilimento_id, codice)` durante UPDATE bulk: fase 1 = parcheggio in pos `(-1, -rank)` + codice `__tmp_<uuid>` (audit soppresso via `audit.batch_tag`), fase 2 = pos e codice finali, fase 3 = INSERT nuovi su celle ora libere, fase 4 = `mappa_passerelle`. DELETE iniziale di clienti_stagionali (FK SET NULL) + ombrelloni (CASCADE su disponibilita). Verifica `stabilimenti.proprietario_id = auth.uid()`. Granted ad `authenticated`. Migration `20260608140000_aggiorna_mappa_ombrelloni.sql`.
...
- `admin-impersona-proprietario` — (Fase 3 Area Admin) genera un magic link per far entrare un admin nel contesto scrivibile di uno stabilimento come il proprietario. JWT verify ON (`config.toml`): richiede sessione admin valida. Input `{ stabilimento_id, redirect_origin }` (origin calcolato client-side da `location.origin`, non hardcodato → funziona su preview Vercel e prod). Verifica caller ∈ `public.admins` (403), risolve `proprietario_id` (404), `auth.admin.getUserById` per l'email, `generateLink({ type:'magiclink', redirectTo: '<origin>/index.html?impersonated=1' })`, logga in `audit_log` (`actor_type='admin'`, `entity_type='auth'`, `action='login'`), ritorna `{ ok, link }`. Il client (admin.html → `ImpersonaButton`) apre il link con `window.open(..., '_blank')`. Banner lato impersonato in `js/main.js` (`?impersonated=1` → flag `sessionStorage['sm_admin_impersonation']` → `#impersonation-banner`). Env: `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`. **Non ancora deployata** (attesa deploy manuale). Vedi `ADMIN_DASHBOARD.md` § Fase 3.
...
Accesso: pagina standalone `https://spiaggiamia.com/admin.html` (React 18 + Babel standalone via CDN + SheetJS per l'export del Log, stack di `devboard.html`; vedi `ADMIN_DASHBOARD.md`). Gate di login separato dalle credenziali proprietario/stagionale: dopo `signInWithPassword` verifica presenza di `auth.uid()` in `public.admins` (`admins_self_select`), altrimenti `signOut`. Quattro tab: **Panoramica** (KPI di piattaforma), **Stabilimenti** (tabella cross-tenant sola lettura), **🗄️ Tabelle** — CRUD generico sulle 6 tabelle business (`profiles`, `stabilimenti`, `ombrelloni`, `clienti_stagionali`, `disponibilita`, `transazioni`), portato dall'ex `js/admin.js` (schema `ADMIN_TABLES` + componenti React `Tabelle`/`RowEditor`) — e **📋 Log** — audit cross-tenant su `audit_log` (versione admin del "Log attività" per-proprietario `js/audit.js`), con filtri stabilimento/date/attore/entità/azione/testo, paginazione server-side ed export Excel; legge via policy `audit_log_select_admin` (accesso a tutti gli stabilimenti, nessuna migration aggiuntiva). Le modifiche passano attraverso RLS (policy `*_admin_*`) — niente service-role in frontend.
...
- **Booking coverage algorithm** (`js/manager.js → computeBookingCoverage`): greedy "scarcity first" — per ogni giorno del range si sceglie UN solo ombrellone tra quelli selezionati liberi, preferendo quello con meno giorni alternativi disponibili (così i "jolly" liberi su tutto il range restano liberi per altri giorni/prenotazioni). Ordine di elaborazione: giorni per scarsità crescente (meno opzioni prima), poi data ascendente. Tiebreak nella scelta dell'ombrellone: `codice` ascendente (`localeCompare 'it'`). Regola di business: 1 prenotazione = 1 ombrellone per giorno; per 2 ombrelloni nello stesso giorno servono 2 prenotazioni separate. Ritorna anche `usedOmbIds`: `openFinalizeBookingModal` mostra un avviso (non bloccante) se l'utente ha selezionato più ombrelloni di quanti ne servano per coprire il range.
