...
> Tutte le migrazioni in `supabase/migrations/` sono state applicate sul DB di produzione al 2026-04-29, **ad eccezione di `20260524000000_ombrellone_codice_mappa.sql`** (aggiunta `codice`/`pos_x`/`pos_y` a `ombrelloni`, `mappa_passerelle` a `stabilimenti`, drop `fila`/`numero`) che deve ancora essere applicata prima del go-live. Cumulative su `reset_stagione`: la 500000 rimpiazza la 300000 (reset dello stato di registrazione + cleanup auth.users per user_id), la 20260426000000 rimpiazza la 500000 (estende la cleanup auth.users anche al match per email + introduce la RPC `unblock_invito_email`).
>
...
- `js/mappa-builder.js`: visual builder 20×20 per disegnare la mappa degli ombrelloni durante l'onboarding. Griglia cliccabile con 3 tipi di cella (`ombrellone`, `passerella`, `vuoto`). Wizard 2 step: Step 1 = disegno layout; Step 2 = assegnazione codici liberi per ogni ombrellone. Funzione `checkOnboardingMappa(stabilimentoId)` chiamata da `js/router.js → loadUserAndRoute` subito dopo login del proprietario: se il conteggio ombrelloni è 0, mostra `#view-mappa-builder` (una view dedicata che nasconde tutto il resto) e ritorna `true` (blocca il normale `showView('manager')`). Al salvataggio inserisce le righe in `ombrelloni` con `{codice, pos_x, pos_y, credito_giornaliero: 10.00}` e aggiorna `stabilimenti.mappa_passerelle`. CSS nella sezione "MAPPA BUILDER" di `styles.css`.
