...
- `js/configurazioni.js`: estratto dal vecchio `js/panoramica.js` per separare la logica della tab Configurazioni (`switchConfigSubtab`, `loadStagione`, `saveStagione`, `renderStagioneSummary`) dalla nuova Panoramica. Lo switcher chiama `avanzateInit()` quando si attiva il sotto-tab `avanzate`.
