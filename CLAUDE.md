...
> Tutte le migrazioni in `supabase/migrations/` sono state applicate sul DB di produzione al 2026-04-25, **eccetto** `20260425300000_reset_stagione_clear_registration.sql` e `20260425500000_reset_stagione_delete_orphan_auth_users.sql` che vanno applicate manualmente prima del prossimo reset stagione (CB) per ottenere il reset dello stato di registrazione e la pulizia delle righe orfane di `auth.users`. La 500000 è cumulativa rispetto alla 300000 (rimpiazza l'intera function `reset_stagione` con la versione finale): se applichi solo la 500000 ottieni comunque il comportamento corretto.
>
...
- `reset_stagione(p_stabilimento_id, p_mantieni_cb boolean)` — crea backup automatico, poi cancella `transazioni` + `disponibilita` dello stabilimento. Se `p_mantieni_cb=true` preserva anagrafica + ombrelloni MA azzera `credito_saldo` e resetta lo stato di registrazione (`user_id=NULL`, `invitato_at=NULL`, `approvato=false`, `rifiutato=false`, nuovo `invito_token`) — i clienti tornano "Mai invitato" e devono essere reinvitati per la nuova stagione; se `false` cancella anche `clienti_stagionali` e `ombrelloni`. Setta `audit.batch_tag` per sopprimere il rumore audit per-riga durante le DELETE/UPDATE massive. Ritorna l'id del backup. **Pulizia `auth.users`**: alla fine cancella le righe orfane di `auth.users` (cascata su `profiles`) per gli utenti che erano collegati ai clienti dello stabilimento e che non risultano più riferiti da altri `clienti_stagionali`, non sono `proprietario` in `profiles`, e non sono in `admins`. Senza questa pulizia il successivo `sb.auth.signUp` con la stessa email del cliente fallirebbe con "User already registered" (vedi migrazione `20260425500000_reset_stagione_delete_orphan_auth_users.sql`). Il check di sicurezza in `js/router.js → loadUserAndRoute` (sign-out automatico se `clienti_stagionali` con `user_id = auth.uid()` non esiste) resta valido come safety net per le sessioni già aperte al momento del reset.
