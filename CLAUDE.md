...
> Tutte le migrazioni in `supabase/migrations/` sono state applicate sul DB di produzione al 2026-04-24, **eccetto** `20260425200000_stagioni_backup.sql` (reset stagione + backup) che va applicata manualmente prima del primo uso della UI di Reset stagione.
>
...
- `reset_stagione(p_stabilimento_id, p_mantieni_cb boolean)` — crea backup automatico, poi cancella `transazioni` + `disponibilita` dello stabilimento. Se `p_mantieni_cb=true` azzera `credito_saldo` dei clienti; se `false` cancella anche `clienti_stagionali` e `ombrelloni`. Setta `audit.batch_tag` per sopprimere il rumore audit per-riga durante le DELETE/UPDATE massive. Ritorna l'id del backup.
